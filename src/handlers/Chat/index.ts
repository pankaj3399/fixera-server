...
export const sendMessage = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const images = Array.isArray(req.body.images)
      ? req.body.images.filter((item: unknown): item is string => typeof item === "string" && item.trim().length > 0)
      : [];
    ...
    const senderRole = userRole === "professional" ? "professional" : "customer";
  ...
  try {
    ...
    await Conversation.findByIdAndUpdate(conversationId, {
      $set: {
        ...
        professionalUnreadCount: isCustomer ? conversation.professionalUnreadCount : 0,
      },
    });
