...
export const getServiceConfigurationForProfessional = async (req: Request, res: Response) => {
    try {
        ...
        const configuration = await ServiceConfiguration.findOne(filter)
            .select('category service areaOfWork pricingModelName pricingModelType pricingModelUnit certificationRequired requiredCertifications projectTypes professionalInputFields includedItems extraOptions conditionsAndWarnings activeCountries');
